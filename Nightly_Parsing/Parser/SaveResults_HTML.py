...
def SaveFinalResultsHTML (Results):
	...
	HTMLFile = '../Nightly_Results/ResultsHTML-' + str(today) + '.html'
	with open(HTMLFile, "w") as output:
		...
		function expand_div ()
		{
			...
			{
				this.parent().children("div").style.display="none";
				this.html('+');
			}
			else
			{
				this.parent().children("div").style.display="inline";
				this.html('-');
			}
		});
		</script>
		...
		output.write("""
		<div>
		<button class="expand_buttons" onclick="expand_div()">+</button>
		<div>Hi There</div> 
		</div>
...
def SaveFinalResultsHTML (Results):
    ...
    #Create a file for outputting the data
    today = datetime.date.today()
    HTMLFile = '../Results/ResultsHTML-' + str(today) + '.html'
    with open(HTMLFile, "w") as output:
